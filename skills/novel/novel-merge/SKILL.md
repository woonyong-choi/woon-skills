...
6. novel 전체는 private/local-only이며 원자료 위치는 `woon resolve repo://knowledge/private/novel`로 확인한다. 별도 Novel workspace나 외부 archive를 만들지 않는다. 쓰기 직전 hash drift면 재계획하고 민감 원본을 외부 MCP·repo·AI에 보내지 않는다. 삭제·commit·push·publish는 별도 요청 없이는 하지 않는다.
7. 반영한 claim·사건·variant와 연결된 catalog·연표·source link의 identity, 누락과 privacy를 확인한다. 공통 catalog·identity·투영 계약이 바뀌면 `repo://knowledge/private/novel`의 `scripts/audit_novel_workspace.py`와 `Wiki/private/novel/**` 투영의 source 경계 audit을 실행한다. 같은 입력의 통과 근거는 재사용한다. 입력의 서로 다른 창작안이 variant ID·상태·차이 이유와 함께 남았는지 확인하고, 실행하지 않은 검증은 완료로 쓰지 않는다.
