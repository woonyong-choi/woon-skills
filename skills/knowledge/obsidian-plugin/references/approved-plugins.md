...
| --- | --- | --- |
| `manta-diagrams` | `woonyong-choi/manta-diagrams` | Manta Diagrams의 exact `0.1.0` build를 `install-local-build`와 receipt로 격리 시험 Vault에서 검증한다. 일반 Mermaid 렌더를 유지하고 명시적인 viewer 열기와 `manta` fence, pan·zoom·SVG export·원문 열람·오류 시 이전 그림 제거·unload를 확인한다. 노트 쓰기·AI 호출·원격 실행은 없다. 원본의 clean HEAD와 origin을 확인하며 Community 등록 승인은 별도로 재조회한다. |
