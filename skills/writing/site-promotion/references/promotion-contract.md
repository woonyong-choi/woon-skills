...
1. `candidate`: 검토할 글·영수증·로컬 후보를 완성한다. 정본 반영·외부 공개 상태와 구분한다.
2. `approved-for-source-write`: 현재 요청 또는 대화에서 해당 candidate·destination·포함 범위의 반영 권한이 확인됐다. 승인된 Vault source·claim·page spec만 갱신할 수 있다.
3. `validated`: 변경 영향에 필요한 저장소 check와 실제 route/render를 확인하고 재사용한 통과 근거와 미검증 계층을 기록했다. 이는 commit·push·deploy 승인이 아니다.
4. `published`: 요청된 외부 공개와 live artifact 검증까지 끝난 상태다. commit·push·deploy는 각각 실제 요청 범위에서 수행한다.

승인 표현의 단어만으로 권한을 인정하거나 배제하지 않는다. 검토 가능한 candidate·destination·포함 범위를 승인한 응답은 해당 범위에 재사용한다. 포괄적인 작성 요청에서 공개 권리나 대상 선택 권한을 추론하지 않으며, 승격 승인을 다른 candidate나 destination으로 확대하지 않는다.
...
```text
승격 상태: <candidate|approved-for-source-write|validated|published>
...
예정 source: <검토 가능한 상대 경로>
권한 경계: 정본 반영·외부 공개 <승인 범위 또는 승인 필요> · commit/push/deploy <요청 범위>
```

반영 뒤에는 실제 source 상대 경로와 실행·재사용한 검증, 미실행 계층을 기록한다. machine-local 절대 경로는 영수증에 넣지 않는다.

## Public source 경계

- 콘텐츠 정본: `repo://knowledge/wiki/`의 기존 정체성. compiler-managed 본문은 source·claim·page spec에서 수정한다.
- 공개 투영: `repo://pages-output/generated/public-content/`. producer가 생성하며 직접 편집하지 않는다.
