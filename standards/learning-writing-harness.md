...
- 일반 개발 detail 문서의 H1은 다시 찾을 수 있는 짧은 정식 키워드로 쓴다. `타입 시스템`, `JVM`, `널 안전성`, `다음 토큰 예측`처럼 개념의 정체성만 제목에 두고, `개요`, `이해하는 방법`, `왜 필요한가` 같은 설명 문장은 첫 문단이 맡는다. 제목은 고정된 책의 장·절 제목, 사람·프로젝트·원자료의 식별자, 정확한 API 이름을 바꾸지 않는다.
- H2는 이번 문단에서 실제로 바뀌는 값·상태·동작·판단 기준을 짧게 잡는다. `한 칸 뒤`, `창의 길이`, `겹치는 창`처럼 이 글의 장면에만 필요한 말을 고르며, `질문`, `실행`, `결과`, `해석`, `확장`, `~할 때`, `~하는 일`을 모든 글에 같은 순서로 반복하지 않는다. 짧은 글은 H2를 억지로 만들지 않는다.
- 약어와 영문은 독자의 기억을 시험하는 장식으로 쓰지 않는다. `JVM`, `TLS`, `BPE`, `RAII`처럼 역할을 바로 알 수 없는 말은 처음에 하는 일을 쉬운 말로 설명한 뒤 원어와 함께 소개한다. 검색·명령·code에서 필요한 정확한 표기는 유지한다.
- `추상화`, `의미론`, `정합성`, `효율적`, `견고한`, `최적`처럼 평가나 추상 명사만으로 문장을 끝내지 않는다. 누가 무엇을 입력·변경·확인하고 무엇이 달라지는지를 먼저 쓴다. 정확한 전문 뜻이 필요하면 관찰한 장면과 함께 좁혀서 설명한다.
- 쉬운 말은 정확한 말을 지우라는 뜻이 아니다. 언어마다 다른 규칙, 표준 용어, code identifier는 바꾸지 않으며, 같은 말을 여러 번 다른 번역어로 바꾸지 않는다. 독자가 본 장면 → 쉬운 역할 설명 → 정확한 이름의 순서를 지킨다.
